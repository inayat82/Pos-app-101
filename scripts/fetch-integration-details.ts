// scripts/fetch-integration-details.ts
// TypeScript version of the integration details fetcher
...
interface CleanedIntegrationData {
  integrationId: string;
  adminId: string;
  accountName: string;
  assignedUserId: string;
...
async function fetchAllIntegrations(): Promise<IntegrationData[]> {
  try {
...
function displayIntegrations(integrations: IntegrationData[]): void {
  console.log('\n🔍 INTEGRATION DETAILS:');
...
function saveIntegrationDetails(integrations: IntegrationData[]): void {
  const outputFile = path.join(__dirname, 'integration-details.json');
  const cleanedIntegrations: CleanedIntegrationData[] = integrations.map(integration => ({
...
async function checkDataConsistency(integrations: IntegrationData[]): Promise<void> {
