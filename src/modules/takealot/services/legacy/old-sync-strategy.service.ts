...
import { takealotProxyService } from '@/lib/services/takealotProxyService';
...
export async function fetchAndSaveTakealotData(
  ...
  do {
    ...
    try {
      ...
      // Use the new proxy service instead of axios
      const response = await takealotProxyService.get(endpoint, apiKey, params, {
        adminId: adminId,
        integrationId: adminId, // Using adminId as integrationId for this legacy service
        requestType: 'manual', // This service doesn't distinguish between manual/cron yet
