import { TakealotRequestConfig, DEFAULT_TAKEALOT_CONFIG } from '@/types/webshare';
import WebshareProxyService from './webshareProxyService';
...
class TakealotApiService {
  ...
  constructor(
    ...
    proxyService?: WebshareProxyService
  ) {
    this.config = { ...DEFAULT_TAKEALOT_CONFIG, ...config };
    this.proxyService = proxyService || new WebshareProxyService();
  }
   ...
   * Set proxy service
   */
  setProxyService(proxyService: WebshareProxyService): void {
  ...
  private updateRateLimit(): void {
    ...
    const minuteTracker = this.rateLimitTracker.get(`minute_${minuteKey}`) || { count: 0, resetTime: (minuteKey + 1) * 60000 };
    minuteTracker.count++;
    this.rateLimitTracker.set(`minute_${minuteKey}`, minuteTracker);

    // Update hour counter
    const hourTracker = this.rateLimitTracker.get(`hour_${hourKey}`) || { count: 0, resetTime: (hourKey + 1) * 3600000 };
    hourTracker.count++;
  ...
  async makeRequest<T = any>(
    ...
    for (let attempt = 0; attempt <= this.config.maxRetries; attempt++) {
      try {
        ...
        if (this.config.useProxy && this.proxyService) {
          // Use proxy for the request
          const proxy = this.proxyService.getNextProxy(this.config.proxyRotation);
          if (proxy) {
            proxyUsed = `${proxy.proxy_address}:${proxy.port}`;
            
            // In a real implementation, this would go through your backend API
            // that handles proxy requests, since browsers can't directly use HTTP proxies
            response = await this.makeProxyRequest(url, requestOptions, proxy);
          } else {
  ...
  getRateLimitStatus(): {
    ...
    return {
      perMinute: {
        current: minuteTracker.count,
        limit: this.config.rateLimit.requestsPerMinute,
        resetTime: minuteTracker.resetTime,
      },
      perHour: {
        current: hourTracker.count,
        limit: this.config.rateLimit.requestsPerHour,
        resetTime: hourTracker.resetTime,
      },
    };
  }
}

export default TakealotApiService;
