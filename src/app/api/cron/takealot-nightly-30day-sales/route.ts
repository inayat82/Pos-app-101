...
export async function GET(request: NextRequest) {
  ...
  try {
    ...
    logId = await cronJobLogger.startExecution({
      cronJobName: 'takealot-nightly-30day-sales',
      ...
      details: 'Automated sync for Last 30 Days sales strategy across all enabled integrations with order_id-based upsert'
    });

    console.log('[Nightly30DaySalesCron] Starting nightly 30-day sales sync');
    
    // Get integrations with nightly 30-day sales sync enabled
    ...
    for (const integrationDoc of integrationsSnapshot.docs) {
      ...
      if (!apiKey) {
        console.warn(`[Nightly30DaySalesCron] No API key for integration ${integrationId}, skipping`);
        continue;
      }
      ...
      try {
        ...
        if (!thirtyDayStrategy) {
          console.log(`[Nightly30DaySalesCron] 30-day nightly strategy not enabled for integration ${integrationId}`);
          continue;
        }
        ...
        console.error(`[Nightly30DaySalesCron] Error processing integration ${integrationId}:`, error);
        results.push({
          integrationId,
          adminId,
          success: false,
    ...
    console.log(`[Nightly30DaySalesCron] ${summaryMessage}`);
    ...
    console.error('[Nightly30DaySalesCron] Fatal error in nightly 30-day sales sync:', error);
