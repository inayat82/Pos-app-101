// src/app/api/cron/takealot-12hourly-all-products/route.ts
...
export async function GET(request: NextRequest) {
  try {
    console.log('Starting takealot-12hourly-all-products cron job');
    
    // Get all Takealot integrations that have the "Fetch All Products (12h)" strategy enabled
    ...
    for (const integrationDoc of integrationsSnapshot.docs) {
      try {
        ...
        // Check if this integration has the "Fetch All Products (12h)" strategy enabled for cron
        ...
        if (preferencesDoc.exists) {
          ...
          const strategy = productStrategies.find((s: any) => s.id === 'prd_all_12h');
          
          if (!strategy?.cronEnabled) {
            console.log(`Skipping integration ${integrationId}: prd_all_12h cron not enabled`);
            continue;
          }
        } else {
        ...
        console.log(`Processing optimized 12-hourly all products sync for integration: ${integrationId}`);
        ...
        console.log(`Completed optimized 12-hourly all products sync for ${integrationId}:`, {
          ...syncResult,
          costSavings
        });
    ...
    console.error('Takealot 12-hourly all products cron job error:', error);
    return NextResponse.json({
      success: false,
      error: error.message,
